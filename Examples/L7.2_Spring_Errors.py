...
data_file = 'L7.2_lectureCode_springData.txt'
data_file2 = 'L7.4_lectureCode_launcherData.txt'
...
def plotData(fileName):
    ...
    xVals *= 9.81  # convert mass to force (F = mg)
...
def testErrors(ntrials=100000, npts=100):
    ...
    pylab.title('Sum of 100 random points -- Triangular PDF (100,000 trials)')
    pylab.xlabel('Sum')
...
def fitData2(fileName):
    ...
    yVals = pylab.array(yVals)
    xVals *= 9.81  # convert mass to force (F = mg)
    extX *= 9.81  # convert mass to force (F = mg)
    pylab.plot(xVals, yVals, 'bo', label='Measured displacements')
    pylab.title('Measured Displacement of Spring')
    pylab.xlabel('|Force| (Newtons)')
    pylab.ylabel('Distance (meters)')
